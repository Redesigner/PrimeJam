...
TArray<FActiveGameplayEffectHandle> UEffectApplicationComponent::ApplyGameplayEffectsToTarget(AActor* Target, FHitResult HitResult)
{
	...
	{
		UE_LOGFMT(LogPrimeJam, Warning, "EffectApplicationComponent {ComponentName} was unable to apply a GameplayEffect to {OtherActorName}. No GameplayEffectHandles were set. Make sure to call SetEffectHandles after spawning the projectile.", GetName(), Target->GetName());
		return Result;
	}
	...
	if (IAbilitySystemInterface* ActorAbility = Cast<IAbilitySystemInterface>(Target))
	{
		for (FGameplayEffectSpecHandle EffectHandle : EffectHandles)
		{
			...
			if (!ActorAbility->GetAbilitySystemComponent())
			{
				...
				UE_LOGFMT(LogPrimeJam, Warning, "Area of Effect {ProjectileName} was unable to apply a GameplayEffect to {OtherActorName}. The target ASC was invalid.", GetName(), Target->GetName());
				return Result;
			}
		...
		for (TSubclassOf<UGameplayEffect> GameplayEffectClass : DefaultEffects)
		{
			...
			{
				UE_LOGFMT(LogPrimeJam, Warning, "Area of Effect {ProjectileName} was unable to apply a GameplayEffect to {OtherActorName}. The GameplayEffectClass was invalid.", GetName(), Target->GetName());
				continue;
			}
